...
def solve(): #LINECTF{warming_up_crypto_YEAH}
    ivVc = b64decode(input('test Command: ').encode())
    iv,c,ct = ivVc[:16], ivVc[16:-16], ivVc[-16:]
    iv_ = c[-16:]
    print('IV...:', b64encode(iv_).decode())
    print('Message...:', b64encode(COMMAND[1]).decode())
    cs = b64decode(input('Ciphertext: '))[-16:]
    print('Enter your command:', b64encode(iv+c+cs).decode())
