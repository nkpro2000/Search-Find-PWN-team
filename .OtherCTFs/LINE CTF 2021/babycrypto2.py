...
def solve(): #LINECTF{echidna_kawaii_and_crypto_is_difficult}
    ivVc = b64decode(input('test Command: ').encode())
    iv,c = ivVc[:16], ivVc[16:]
    iv = bytearray(iv)
    iv[9:13] = map(lambda x:x[0]^x[1], zip(iv[9:13], COMMAND[0])) # db1[0] = iv[0] ^ t
    iv[9:13] = map(lambda x:x[0]^x[1], zip(COMMAND[1], iv[9:13])) # s ^ db1[0]
    print('Enter your command:', b64encode(iv+c).decode())        # s ^ db1[0] ^ db1[0] == s
